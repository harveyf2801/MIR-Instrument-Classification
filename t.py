...
for filename in os.listdir(audio_folder):
  if filename.endswith(".wav"):
    ...
    sf = librosa.feature.spectral_flatness(y=padded_audio, n_fft=2**10, win_length=2**10, hop_length=2**9)

    # Calculate the mean, standard deviation, skewness, and kurtosis along the 0 axis for MFCC features
    mfcc_mean = np.mean(mfcc, axis=0)
    mfcc_std = np.std(mfcc, axis=0)

    # Calculate the mean, standard deviation, skewness, and kurtosis along the 0 axis for spectral flatness
...
# Apply PCA
pca = PCA()
mfcc_features_pca = pca.fit_transform(mfcc_features_scaled)
# mfcc_features_pca, explained = data_anaysis.pca(mfcc_features_scaled, 2)
# mfcc_features_pca = np.real(mfcc_features_pca)

# Split the data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(mfcc_features_pca, target_ids, test_size=0.2, random_state=50)
...
print("Best Dimensions:", best_dimensions)
